  ...
  <div class="article-detail">
    ...
    <div class="article-header">
      ...
      <div class="info">
        ...
        <div
          class="view-info"
        >文章数 {{card.articleQuantity || 0}}&nbsp;&nbsp;&nbsp;视频数 {{card.videoQuantity}}&nbsp;&nbsp;&nbsp;浏览量 {{card.pageView || 0}}</div>
      </div>
    ...
    <div class="show-card" v-if="goods.length > 0">
      <div class="block-item-title">
        <i class="iconfont iconcard_pause"></i>
        推荐的商品
      </div>
...
export default {
  ...
  methods: {
    ...
    changeToMe() {
      ...
      toast.show();
      axios
        .post(this.$apiConfig.changeToMy, params)
        .then(res => {
          if (res.data.code == 0) {
            toast.hide();
            window.location.replace(
              `${this.getCrtUrl()}?docId=${this.docId}&cardId=${
                this.meCardId
              }#/article`
            );
          }
        ...
        .then(() => {
          if (this.env != "h5") {
            uni.redirectTo({
              url: `/pages/article-detail/article-detail?share_cardId=${
                this.meCardId
              }&share_type=article&docId=${this.docId}`
            });
          }
        });
    },
